...
- **JDK**: 21
- **Node.js**: >= 18 (用於 ECPay 金流模組)
- **MySQL**: 8.0.x
  - 專案已配置支援連線至 **Railway 雲端資料庫**，可透過更改 `application.properties` 切換本地端或雲端。
  - 時區: `Asia/Taipei`
  - 編碼: `UTF-8`
- **Email SMTP**: 需配置 Gmail SMTP (Port 587, STARTTLS) 用於發送驗證碼與重置密碼信件。

### 部署與啟動說明

1. **資料庫準備**：確保 MySQL (本機或 Railway) 執行中並具備 `product_shop` schema。
2. **後端啟動**：
   - 確認 `src/main/resources/application.properties` 設定 (資料庫、Gmail SMTP 密碼、JWT Secret)。
   - 執行 `mvn spring-boot:run`，服務預設執行於 `http://localhost:8080`。
3. **金流服務啟動**：
   - 進入 `/ecpay` 目錄。
   - 複製 `.env.sample` 為 `.env` 並填寫綠界測試環境的 HashKey 與 HashIV。
   - 執行 `npm install` 安裝相依套件。
   - 執行 `npm start` 啟動 Node.js 金流服務。

---
*此文檔為 ProductShop 全端電商專案之技術架構指南，展現了前後端分離、外部 API 容錯整合、動態定價運算以及獨立金流模組等架構思維。*